...
pub fn parse_sped_fields(
    ...
    // SPED line Format: |REG|FIELD1|FIELD2|...| (starting with a delimiter),
    // splitting by | results in an empty string at index 0 and the register code at index 1.
    // |0000|... -> ["", "0000", ...]

    let registro: Option<&str> = fields
        .get(1)
        .filter(|s| s.len() == 4 && s.chars().all(|c| c.is_ascii_alphanumeric()))
        .copied(); // Converte Option<&&str> -> Option<&str>

    // Basic validation: A valid SPED record line must have at least 4 fields:
    // '|', REG, FIELD1, FIELD2, ..., '|'.
    // And the record type (registro) must not be empty.
    if registro.is_none() || fields.len() < 4 {
        let msg = "Line with empty record identifier or insufficient number of fields.";
        if fields.len() > 1 && line.is_ascii() {
            warn!(
    ...
    // TÉCNICA DE OTIMIZAÇÃO: Normalização na Stack
    // Como SPED sempre tem 4 chars, criamos um buffer fixo na stack.
    // Isso evita o custo de `to_uppercase()` que gera uma String na Heap.

    // 1. Extrai o slice de bytes (Custo zero)
    let bytes: &[u8] = registro.unwrap_or_default().as_bytes();

    // 2. Cria o array transformando em maiúsculo (Alocação na Stack)
    // O compilador sabe que 'i' vai de 0 a 3.
    let reg_buffer: [u8; 4] = std::array::from_fn(|i| {
        // Como validamos len == 4 antes, isso é seguro e rápido.
        bytes[i].to_ascii_uppercase()
    });

    // 3. Converte para &str
    // Como to_ascii_uppercase gera ASCII garantido, isso nunca falha.
    // unwrap_or_default é usado apenas como boa prática de robustez.
    let registro_uppercase: &str = std::str::from_utf8(&reg_buffer).unwrap_or_default();
