use crate::{
    DocsFiscaisNew, EFDError, EFDResult, InformacoesNew, NEWLINE_BYTE, Registro0000, SpedContext,
    SpedFile, SpedRecord, get_string_utf8, info_new::process_block_lines,
    parser::parse_sped_fields,
};

use chrono::Datelike;
use claudiofsr_lib::{FileExtension, get_style, num_digits, open_file};
...
/// documentos fiscais, DocsFiscaisNew.
///
...
pub fn analyze_one_file_new(
    ...
    let (all_docs, all_messages): (Vec<DocsFiscaisNew>, String) = blocks_to_process
        .into_par_iter()
...
fn update_progressbar(
    ...
    // Atualizar a cada 10 linhas em processamento paralelo causa muita contenção no Atomic interno.
...
/// Run tests with:
/// cargo test -- --show-output analyze_one_new_tests
#[cfg(test)]
#[path = "tests/analyze_one_new_tests.rs"]
mod analyze_one_new_tests;
