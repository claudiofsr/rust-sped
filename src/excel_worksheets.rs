...
use claudiofsr_lib::{StrExtension, get_style, match_cast};
...
use crate::{
    EFDResult, IndicadorOrigem, REGEX_ALIQ, REGEX_CENTER, REGEX_DATE, REGEX_VALUE, TipoDeCredito,
    TipoDoItem, TipoOperacao, display_cst, display_natureza,
};
    ...
    lines
        ...
        .try_for_each(|line| -> EFDResult<()> {
            line
                .iter()
                .enumerate()
                .try_for_each( |(index, (field_name, field_value))| -> EFDResult<()> {
                    let column_number: u16 = index.try_into()?;
...
fn auto_fit<'de, T>(worksheet: &mut Worksheet, lines: &[T], column_names: &[&str]) -> EFDResult<()>
where
    T: Serialize + Deserialize<'de> + InfoExtension + Iterable, // + Sync + Send
{
        ...
        .for_each(|(index, (field_name, field_value))| {
            ...
            let field_value_len: usize = match_cast!( field_value {
                    ...
                    val.as_ref().map(|s| {
                        ...
                        match field_name {
                            ...
                            "natureza_bc" => {
                                ...
                                string.chars_count() * 74 / 100  // Ajustes
                            },
                            _ => {
                ...
                val as Option<IndicadorOrigem> => {
                    val.as_ref().map(|s| s.to_string().chars_count() * 88 / 100)
                },
                val as Option<TipoOperacao> => {
                    val.as_ref().map(|s| s.to_string().chars_count())
                },
                val as Option<TipoDeCredito> => {
                    val.as_ref().map(|s| s.to_string().chars_count() * 84 / 100)
                },
