...
macro_rules! store_pis {
    ($mgr:expr, $rec:expr, $ty:ty) => {
        // 1. Tenta fazer o downcast para o tipo específico (ex: RegistroC191)
        if let Ok(reg) = $rec.downcast_ref::<$ty>() {
            // Cria a struct auxiliar aqui
            let opt_key = $crate::extractor::CorrelationKey::new(reg.cst_pis, reg.vl_item);

            // 2. Tenta criar a chave. Se faltar CST ou Valor, ignora silenciosamente.
            if let Some(key) = opt_key {
                let ctx = $crate::extractor::CorrelationCriteria {
                    cfop: reg.get_cfop(),
                ...
                $mgr.store(key, ctx, reg.aliq_pis, reg.vl_pis);
            }
        }
    };
}
