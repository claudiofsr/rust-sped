...
/// Macro para implementar despacho estático de métodos em enums de blocos.
///
/// Gera as funções line_number() e registro_name() para cada registro.
#[macro_export]
macro_rules! impl_bloco_methods {
    ($enum_name:ident { $($registro:ident => $reg_name:expr),* $(,)? }) => {
        impl $enum_name {
            #[inline(always)]
            pub fn nivel(&self) -> u16 {
                match self {
                    $(Self::$registro(r) => r.nivel,)*
                }
            }
            ...
            pub fn bloco(&self) -> char {
                // $reg_name.chars().next().unwrap()
                match self {
                    // Extrai o primeiro char da string literal em tempo de compilação
                    $(Self::$registro(_) => $reg_name.chars().next().unwrap_or('?'),)*
                }
            }
        ...
        // Implementação do Trait exigido pelo SpedFile e Extratores
        impl $crate::traits::SpedRecordTrait for $enum_name {
            #[inline]
            fn nivel(&self) -> u16 {
                self.nivel()
            }
