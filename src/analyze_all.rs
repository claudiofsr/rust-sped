...
use crate::{
    ...
    OUTPUT_FILENAME, TipoDeOperacao, analyze_one_file,
    args::Arguments,
    create_xlsx,
...
pub fn executar_programa(args: &Arguments, write: &mut dyn Write) -> EFDResult<Vec<DocsFiscais>> {
    ...
    // 6.1. Criamos dois buffers em memória para capturar a saída de cada função
    let mut buffer_xlsx = Vec::new();
    let mut buffer_csv = Vec::new();

    // 6.2. Executamos em paralelo
    // O rayon::join permite capturar erros de ambos os lados
    let (res_xlsx, res_csv) = join(
        || {
            if args.not_print_xlsx {
                Ok(())
            } else {
                create_xlsx(
                    &filtered_lines,
        ...
        || {
            if args.print_csv {
                print_csv_file(&filtered_lines, &mut buffer_csv)
            } else {
                Ok(())
            }
        },
    ...
    // 6.3. Verificamos se houve erro antes de escrever no output principal
    res_xlsx?;
    res_csv?;

    // 6.4. Consolida os buffers no 'write' original (sequencialmente)
    if !buffer_xlsx.is_empty() {
        write.write_all(&buffer_xlsx)?;
    }
    if !buffer_csv.is_empty() {
        write.write_all(&buffer_csv)?;
    }
...
fn should_keep_record(doc: &DocsFiscais, args: &Arguments) -> bool {
    ...
    if args.excluir_saidas && doc.tipo_de_operacao == Some(TipoDeOperacao::Saida) {
        return false;
    }
...
fn analyze_all_files(
    ...
    let mut all_info: Vec<Informacoes> = arquivos_efd
        ...
        .filter_map(|(index, arquivo)| {
            ...
            match analyze_one_file(&multiprogressbar, arquivo, index, total) {
                // Condição de Sucesso:
...
fn consolidar_resultados(
    ...
    let (cst_result, nat_result) = rayon::join(
        ...
        || analise_dos_creditos::consolidar_natureza_da_base_de_calculo(args, database),
    );
