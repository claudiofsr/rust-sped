    ...
    let filtered_lines: Vec<DocsFiscais> = all_lines
        ...
        .filter(|docs_fiscais| {
            ...
            !(args.excluir_saidas
                ...
                && docs_fiscais.tipo_de_operacao == Some(TipoOperacao::Saida))
        })
        .filter(|docs_fiscais| {
...
fn analyze_all_files(
    ...
    all_info.par_sort_by_key(|informacoes| {
        (
            informacoes.cnpj_base.to_owned(),
            informacoes.periodo_de_apuracao.year(),
            informacoes.periodo_de_apuracao.month(),
        )
    });
    ...
    let (pa_total, vec_all_data): (Vec<NaiveDate>, Vec<Vec<DocsFiscais>>) = all_info
        .into_iter()
        .enumerate()
        .map(|(index, informacoes)| {
            writeln!(
                write,
                "EFD {:02}: {}",
                index + 1,
                informacoes.all_docs[0].arquivo_efd
            )?;
            writeln!(
                write,
            ...
            write!(write, "{}", informacoes.messages)?;

            consolidar_resultados(args, &informacoes.all_docs, true, &mut write)?;

            Ok((informacoes.periodo_de_apuracao, informacoes.all_docs))
        })
