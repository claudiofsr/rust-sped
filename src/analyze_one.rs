...
use crate::{
    ...
    obter_cod_da_natureza_da_bc,
    obter_modelo_do_documento_fiscal,
};
...
pub fn analyze_one_file(
    registros_efd:  &HashMap<&str, HashMap<u16, (&str, &str)>>,
    dispatch_table: &HashMap<&str, FuncaoLerRegistro>,
    multiprogressbar: &MultiProgress,
    arquivo: &PathBuf,
    index: usize,
    total: usize,
) -> Result<Informacoes, Box<dyn Error>> {

    let mut info: Info = match get_file_info(
        registros_efd,
        dispatch_table,
        multiprogressbar,
        arquivo,
        ...
        Ok(value) => value,
        Err(error) => {
            eprintln!("Erro na função get_file_info()!");
            eprintln!("Arquivo: {}", arquivo.display());
            panic!("{error}");
        },
    };

    let vec_docs_fiscais: Vec<DocsFiscais> = match parse_file_info(&mut info) {
        Ok(value) => value,
        Err(error) => {
            eprintln!("Erro na função parse_file_info()!");
            eprintln!("Arquivo: {}", arquivo.display());
            panic!("{error}");
        },
    };
...
fn get_file_info(
    ...
    let style = get_style(0, 0, 38).expect("get_style error!!!");
    ...
    let iter1 = 1..; // changing the initial value from zero to one
    ...
    iter1.zip(iter2)
        .map(|(line_number, vec_bytes)| (line_number, get_string_utf8(vec_bytes.trim(), line_number, arquivo)))
        .map(|(_line_number, line)| (_line_number, split_line(line)))
        ...
        .try_for_each(|(line_number, campos)| -> Result<(), Box<dyn Error>> {
            ...
            if let (true, Some(naive_date)) = (empty_msg, info.pa) {
                let mes: u32 = naive_date.month();
                let ano: i32 = naive_date.year();
                let msg: String = format!("EFD Contribuições nº {file_number:>num_len$} de {mes:02}/{ano:04}");
    ...
    T: Deref<Target=str> + std::fmt::Debug,
{
    ...
    for valor in campos {
        ...
        let Some(&(campo, tipo)) = registros_efd[registro].get(&index) else {
            eprintln!("Registro {registro}: índice {index} não definido!");
            process::exit(1);
        };
...
/// Formatar n° de casas decimais de campos que contêm valores ou alíquotas
...
fn formatar_casas_decimais (
    ...
    if tipo == "C" || !valor.contains_some_digits() {
        return valor;
    }
    ...
    if tipo == "Valor" {
        decimal = Some(DECIMAL_VALOR);
    } else if tipo == "Aliquota" {
        decimal = Some(DECIMAL_ALIQ);
    }

    if let Some(dec) = decimal {
        let my_number: Result<f64, _> = valor
            .replace('.',"")
            .replace(',',".")
            .parse::<f64>();

        valor_formatado = match my_number {
            Ok(number) => {
                format!("{number:0.dec$}")
            },
            Err(error) => {
                eprintln!("Linha nº {num_line} do arquivo '{arquivo:?}'.");
                eprintln!("Erro na conversão de '{valor}' para Valor Numérico.");
                panic!("Error:{error}")
            },
        };
    }
...
pub fn parse_file_info(info: &mut Info) -> Result<Vec<DocsFiscais>, Box<dyn Error>> {
    ...
    let database: Vec<DocsFiscais> = info.completa
        .iter()
        .filter(|&(_num_linha_efd, hashmap)| !hashmap.is_empty())
        .map(|(&num_linha_efd, hashmap)|
    {
        /*
        ...
        */
        ...
        let estab_cnpj = match hashmap.get("estab_cnpj") {
            Some(value) => value,
            None => panic!("Não foi encontrado o CNPJ do estabelecimento na linha nº {num_linha_efd} do arquivo {arquivo_efd}"),
        };
        let estab_nome = match hashmap.get("estab_nome") {
            Some(value) => value,
            None => panic!("Não foi encontrado o Nome do estabelecimento na linha nº {num_linha_efd} do arquivo {arquivo_efd}"),
        };
        ...
        let tipo_de_operacao: Option<u16> = match hashmap.get("tipo_de_operacao").and_then(|v| v.parse::<u16>().ok()) {
            Some(num_inteiro) => Some(num_inteiro),
            None => {
                match cst_cofins {
        ...
        let natureza_bc: Option<u16> = match hashmap.get("NAT_BC_CRED") {
            Some(nat) => nat.parse::<u16>().ok(),
            None => {
                obter_cod_da_natureza_da_bc(&cfop, cst_cofins)
            }
        };

        // Indicador da origem do crédito: 0 – Operação no Mercado Interno ; 1 – Operação de Importação
        let indicador_de_origem: Option<u16> = match hashmap.get("IND_ORIG_CRED").and_then(|v| v.parse::<u16>().ok()) {
            Some(num_inteiro) => Some(num_inteiro),
            None => {
...
";
    ...
    fn make_file(filename: &str, all_lines: &str) -> Result<(), Box<dyn Error>> {
    ...
    #[test]
    fn analisar_linhas() -> Result<(), Box<dyn Error>> {
        ...
        let mut campos: Vec<String> = split_line(line);
    ...
    fn strip_delimiter() {
        ...
        let campos: Vec<String> = split_line(line);
    ...
    #[test]
    fn test_analyze_one_file() -> Result<(), Box<dyn Error>> {
    ...
    #[test]
    fn open_windows_1252_encoding_file_v1() -> Result<(), Box<dyn Error>> {
        ...
        // https://users.rust-lang.org/t/force-enumerate-to-u64
        let iter1 = 1..;
        let iter2 = BufReader::new(file).split(NEWLINE_BYTE).map_while(Result::ok);

        let lines: Vec<(usize, Vec<String>)> = iter1.zip(iter2)
            .map(|(line_number, vec_bytes)| (line_number, get_string_utf8(vec_bytes.trim(), line_number, &path)))
            .map(|(_line_number, line)| (_line_number, split_line(line)))
            .collect();
    ...
    #[test]
    fn open_windows_1252_encoding_file_v2() -> Result<(), Box<dyn Error>> {
        // cargo test -- --show-output open_windows_1252_encoding_file
