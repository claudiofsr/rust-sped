...
use crate::{
    ...
    REGEX_REMOVE_NON_DIGITS, Tipo, TipoDeCredito, TipoDeRateio, TipoDoItem, TipoOperacao, ToCNPJ,
...
fn update_progressbar(
    ...
    if let (true, Some(periodo_de_apuracao)) = (&empty_msg, info.pa) {
        ...
        let msg: String =
            format!("EFD Contribuição nº {file_number:>num_len$} de {mes:02}/{ano:04}");
    ...
    let result_database: EFDResult<Vec<DocsFiscais>> = info
        ...
        .map(|(&num_linha_efd, hashmap)| -> EFDResult<DocsFiscais> {
            ...
            */
            ...
            let tipo_item: Option<TipoDoItem> = info
                .produtos
                .get(cod_item)
            ...
            let mut docs_fiscais = DocsFiscais {
                ...
                trimestre: Some(periodo_de_apuracao.quarter()),
...
fn determinar_tipo_de_credito(
    ...
    match (indicador_de_origem, cst_cofins) {
        ...
        (Some(IndicadorOrigem::MercadoInterno), Some(50..=56)) => {
            // Verifica se as alíquotas correspondem exatamente às aliquotas básicas (1.65% e 7.6%)
