use crate::{
    ...
    Registro0000, SpedContext, SpedFile, SpedRecord, extractor::process_block_lines,
...
pub fn read_and_parse_file(
    ...
    let parallel_results = lines_iter
        ...
        .scan(false, |parar_proxima_iteracao, (idx, line_result)| {
            ...
            // .map_err() é "lazy" [a closure só será executada se houver um erro de leitura]
            // path.to_path_buf() só será chamado se line_result for Err.
            let line_result = line_result.map_err(|e| EFDError::InOutDetalhado {
                source: e,
                path: path.to_path_buf(),
                line_number: idx + 1,
            });
    ...
    I: Iterator<Item = (usize, std::io::Result<Vec<u8>>)>,
{
    ...
    for (idx, line_result) in lines_iter.by_ref() {
        let line_number = idx + 1;
        let bytes = line_result.map_err(|e| EFDError::InOut {
            source: e,
            path: path.to_path_buf(),
        })?;
