...
fn test_correlation_manager_storage_and_retrieval() {
    ...
    let result = mgr.resolve(Some(50), Some(vl_item), corr_ctx);
...
fn test_correlation_manager_partial_strong_key() {
    ...
    let result = mgr.resolve(Some(1), Some(dec!(100.0)), corr_ctx);
...
fn test_determinar_tipo_de_credito() {
    ...
    builder.doc.cst = Some(50);
    builder.doc.aliq_pis = Some(1.65);
    builder.doc.aliq_cofins = Some(7.6);
    builder.doc.cod_credito = None;
    builder.doc.indicador_de_origem = Some(IndicadorOrigem::Importacao);
    ...
    builder.doc.indicador_de_origem = Some(IndicadorOrigem::MercadoInterno);
    ...
    builder.doc.aliq_pis = Some(0.65);
    builder.doc.aliq_cofins = Some(3.0);
    builder.doc.indicador_de_origem = Some(IndicadorOrigem::MercadoInterno);
