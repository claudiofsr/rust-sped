...
fn consolidar_keys(linhas: &[DocsFiscais]) -> HashMap<Keys, Values> {
    ...
    let map_reduce: HashMap<Keys, Values> = linhas
        ...
        //.filter(|&linha| linha.tipo_de_operacao == Some(1) || linha.tipo_de_operacao == Some(2))
        //.filter(|&linha| linha.tipo_de_operacao.is_some_and(|x| x <= 2)) // 1: Entrada, 2: Saída
        .filter(|&linha| operacoes_selecionadas.contains(&linha.tipo_de_operacao)) // 1: Entrada, 2: Saída
        .map(get_keys_values)
...
mod tests {
    ...
    // cargo test -- --show-output operacoes_com_vetores
        ...
        vec_a.extend(vec_b);
        println!("vec_a: {:?} extend", vec_a);

        vec_a.sort_unstable();
        println!("vec_a: {:?} sort", vec_a);

        vec_a.dedup();
        println!("vec_a: {:?} dedup", vec_a);
