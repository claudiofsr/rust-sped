...
impl Valores {
    ...
    pub fn distribuir_conforme_rateio(
        ...
        // Encontra o tipo de rateio e o valor simultaneamente
        let resultado = cod_credito
            .and_then(TipoDeRateio::from_codigo_credito)
            .zip(valor_opt);

        if let Some((rateio, valor)) = resultado {
            *self.obter_campo_de_rateio_mut(rateio) = valor;
        }
    }
}
...
pub fn consolidar_natureza_da_base_de_calculo(
    ...
    for (k, v) in chaves_consolidadas {
        if k.cst.eh_receita_bruta(args.excluir_cst_49) {
...
fn distribuir_descontos_rateados(linhas: &[DocsFiscais]) -> HashMap<Chaves, Valores> {
    ...
    consolidar_registros(
        ...
        |linha| {
            ...
            let offset = match registro.as_ref() {
                "M100" | "1100" => 1,
                "M500" | "1500" => 5,
                _ => panic!("Erro: Registro {registro} não suportado em descontos!"),
            };
