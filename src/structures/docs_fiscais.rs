...
use crate::{
    InfoExtension, indicador_de_origem_to_str, serialize_natureza, serialize_tipo_de_credito,
...
pub struct DocsFiscais {
    ...
    #[serde(
        rename = "Indicador de Origem",
        //deserialize_with = "csv::invalid_option",
        serialize_with = "serialize_indicador_de_origem",
    )]
...
impl DocsFiscais {
    ...
    pub fn get_values(&self) -> Vec<String> {
        ...
        vec![
            ...
            self.particante_cnpj.clone(),
            self.particante_cpf.clone(),
            self.particante_nome.clone(),
...
where
    S: Serializer,
{
    let str = indicador_de_origem_to_str(codigo);
...
mod option_mes {
    ...
    pub fn serialize<S>(mes: &Option<u32>, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        match mes {
