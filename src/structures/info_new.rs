...
use crate::{
    ALIQ_BASICA_COF, ALIQ_BASICA_PIS, DecimalExt, DocsFiscaisNew, EFDError, FloatExt,
    IndicadorOrigem, MesesDoAno, SpedContext, SpedFile, SpedRecord, SpedRecordTrait, StringParser,
    TipoDeCredito, TipoDeRateio, TipoDoItem, TipoOperacao, blocos::*, capture_cnpj, cred_presumido,
    impl_dopai, impl_filho, obter_cod_da_natureza_da_bc, obter_modelo_do_documento_fiscal,
...
// Ex: (CFOP + Nat_BC_Cred + Part + Cta) = 15 (Máxima especificidade)
...
impl CorrelationEntry {
    ...
    fn new(ctx: CorrelationContext, value: CorrelationValue) -> Self {
        Self {
            cfop: ctx.cfop,
            nat_bc_cred: ctx.nat_bc_cred,
            part: ctx.part.map(Arc::from),
            cod_cta: ctx.cod_cta.map(Arc::from),
            vl_bc: ctx.vl_bc,
            value,
        }
    }
    ...
    /// - ctx: CorrelationContext consulta informacoes da COFINS
    ///
    ...
    #[inline(always)]
    fn calculate_score(&self, ctx: CorrelationContext) -> u8 {
    ...
    /// Verifica igualdade exata para o 'store' (evitar duplicidade)
    fn is_exact_match(&self, ctx: CorrelationContext) -> bool {
        self.cfop == ctx.cfop
            && self.nat_bc_cred == ctx.nat_bc_cred
            && self.part.as_deref() == ctx.part
            && self.cod_cta.as_deref() == ctx.cod_cta
            && self.vl_bc == ctx.vl_bc
    }
}
...
impl CorrelationManager {
    ...
    /// Armazena dados de PIS para futura correlação com COFINS.
    pub fn store(
        &mut self,
        key: CorrelationKey,
        ctx: CorrelationContext, // informacoes obtidas dos registros
        ...
        // 1. Atualização (Update): Se já existe regra idêntica, atualiza valores.
        if let Some(entry) = bucket.iter_mut().find(|e| e.is_exact_match(ctx)) {
            entry.value = value;
        } else {
            // 2. Inserção (Insert): Cria nova regra.
            let new_entry = CorrelationEntry::new(ctx, value);

            // Não ordenamos aqui. Deixamos para o 'resolve' decidir quem ganha.
    ...
    pub fn resolve(
        ...
        */
        ...
        bucket
            .iter()
            // Calcula o score e retorna o valor associado ao maior score.
            // Se o score for 0 em todos, retorna o primeiro (ou último) dependendo da impl,
            // mas logicamente score 0 significa "Generic Match" se a regra não tiver restrições.
...
impl<'a> DocsBuilder<'a> {
        ...
        F: RegistroFilho + ?Sized,
    {
        ...
        self.doc.valor_iss = filho.get_valor_iss().or(self.header.vl_iss).to_f64_opt();
        ...
        F: RegistroFilho + ?Sized,
    {
        ...
        if let Some(pis_data) = manager.resolve(
            ...
            self.doc.aliq_pis = pis_data.aliq_pis.to_f64_opt();
            self.doc.valor_pis = pis_data.vl_pis.to_f64_opt();
    ...
    fn resolve_indicador_de_origem(mut self) -> Self {
        self.doc.indicador_de_origem = self
            ...
            .or_else(|| {
                self.doc
                    .cod_credito
                    .and_then(|cod| IndicadorOrigem::from_u16(cod % 100))
            })
    ...
    /// Utiliza o `IndicadorOrigem` resolvido para decidir entre fluxos de Importação ou Mercado Interno.
    ...
    fn calcular_tipo_de_credito(&self) -> Option<TipoDeCredito> {
        ...
        // 0. Validação Prévia
        if cst_cofins.is_some_and(|c| !(50..=66).contains(&c)) {
            return None;
        }
        ...
        if matches!(origem, Some(IndicadorOrigem::Importacao)) {
...
/// Chave para correlação exata no Bloco M.
/// Como a busca é exata, usamos uma struct Hashable simples, sem "Context" complexo.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
struct CreditKey {
    cod_cred: u16,   // M100/M500
    nat_bc: u16,     // M105/M505
    cst: u16,        // M105/M505
    val_bc: Decimal, // M105/M505 (Decimal é Hashable se normalizado)
}

#[derive(Default)]
pub struct CreditCorrelationManager {
    /// Mapeia a chave única do crédito para a Alíquota de PIS encontrada.
    cache: HashMap<CreditKey, Decimal>,
}

impl CreditCorrelationManager {
    /// Armazena dados vindos do PIS (M100/M105)
    pub fn store(
        &mut self,
        cod_cred: Option<u16>,
        nat_bc: Option<u16>,
        cst_pis: Option<u16>,
        val_bc: Option<Decimal>,
        aliq_pis: Option<Decimal>,
    ) {
        // Bloco M exige rigor: se faltar dado, não dá para correlacionar.
        if let (Some(cc), Some(nb), Some(cst), Some(vbc), Some(aliq)) =
            (cod_cred, nat_bc, cst_pis, val_bc, aliq_pis)
        {
            let key = CreditKey {
                cod_cred: cc,
                nat_bc: nb,
            ...
            self.cache.insert(key, aliq);
        }
    }

    /// Tenta recuperar a Alíquota de PIS para usar no COFINS (M500/M505)
    pub fn resolve(
        &self,
        cod_cred: Option<u16>,
        nat_bc: Option<u16>,
        ...
        let (Some(cc), Some(nb), Some(cst), Some(vbc)) = (cod_cred, nat_bc, cst_cofins, val_bc)
        else {
            return None;
        };
        ...
        let key = CreditKey {
            cod_cred: cc,
            nat_bc: nb,
            cst, // Assume-se que CST PIS == CST COFINS na correlação de crédito
            val_bc: vbc,
        };
...
struct BlocoM<'a> {
    ...
    cred_mgr: CreditCorrelationManager,
}
impl<'a> BlocoM<'a> {
    /// Lógica de Correlação de Alíquotas.
    ///
    /// Tenta resolver a alíquota de PIS usando:
    /// 1. Tabela Estática (se houver aliq_cofins)
    /// 2. Cache Dinâmico (construído via M105)
    fn resolve_pis_aliq(
        &self,
        ...
        // 1. Tenta Tabela Estática
        aliq_cofins
            .and_then(|aliq| obter_pis_da_tabela_estatica(&aliq))
            // 2. Tenta Cache Dinâmico
            .or_else(|| {
                self.cred_mgr
                    .resolve(pai.cod_cred, f.nat_bc_cred, f.cst_cofins, f.vl_bc_cofins)
            })
    }
        ...
        for record in records {
            ...
            match g.registro_name() {
                ...
                "M105" => {
                    ...
                    if let (Ok(f), Some(pai)) = (record.downcast_ref::<RegistroM105>(), self.m100) {
                        // STORE: Lógica encapsulada
                        self.cred_mgr.store(
                            pai.cod_cred,
                            f.nat_bc_cred,
                            f.cst_pis,
                ...
                "M505" => {
                    // M505: Consome Tabela ou Cache
                    if let (Ok(f), Some(pai)) = (record.downcast_ref::<RegistroM505>(), self.m500) {
                        // RESOLVE: Chamada limpa
                        let aliq_pis = self.resolve_pis_aliq(pai.aliq_cofins, f, pai);

                        // Tratamento de Erro (Log)
                        if aliq_pis.is_none() {
                            let erro = EFDError::CorrelationPISCOFINS {
                                arquivo: ctx.path.clone(),
                                linha_num: f.line_number,
                                registro: "M505".to_string(),
                                aliq_cofins: pai
                                    .aliq_cofins
                                    .map(|v| v.to_string())
                                    .unwrap_or_else(|| "N/A".to_string()),
                            };
                            // Imprime o erro formatado (mantendo comportamento original de não abortar)
                            eprintln!("{}", erro);
                        }
                        ...
                        b.doc.aliq_cofins = pai.aliq_cofins.to_f64_opt();
...
mod mappers {
    ...
    fn generate_adjustments(
        ...
        // Mapeamento: (Valor, TipoOperacao, Multiplicador de Sinal)
    ...
    pub fn build_ctrl_credito(
        ...
        ctx: &SpedContext,
    ) -> (Option<DocsFiscaisNew>, Option<String>) {
        // Obter Tributo
        ...
        // 2. Validação de Valor: Ignora valores zerados
        let credito_descontado = vl_cred_desc.abs_f64();
        if credito_descontado.eh_zero() {
            return (None, None);
        }
