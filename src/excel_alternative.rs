...
use crate::{
    ...
    MesesDoAno, OUTPUT_DIRECTORY, OUTPUT_FILENAME, display_aliquota, display_cst,
...
fn add_row_efd(
    ...
    let indicador_de_origem = colunas.indicador_de_origem.to_string();
    let tipo_de_operacao = colunas.tipo_de_operacao.to_string();
    let descricao_do_cst = obter_descricao_do_cst(&colunas.cst);
    let descricao_do_cfop = obter_descricao_do_cfop(&colunas.cfop);

    // Tipo de Crédito com código (Ex: "01 - Alíquota Básica")
    let descricao_do_tipo_de_credito = colunas
        .tipo_de_credito
        .map(|t| t.descricao_com_codigo())
        .unwrap_or_default();

    let descricao_do_tipo_de_item = colunas
        .tipo_item
        .map(|t| t.descricao_com_codigo())
        .unwrap_or_default();
...
fn add_row_nat(
    ...
    // Tipo de Crédito sem código (Ex: "Alíquota Básica")
    let descricao_do_tipo_de_credito = colunas
        .tipo_de_credito
        .map(|t| t.to_string())
        .unwrap_or_default();
