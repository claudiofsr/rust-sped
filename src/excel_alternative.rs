...
fn add_row_efd(
    ...
    let f = |n: &str| {
        fmt.get(n)
            .ok_or_else(|| EFDError::FormatNotFound(n.to_string()))
    };
    ...
    RowWriter::new(sheet, row, width_map)
        ...
        .cell(col.cod_credito, f("integer")?)?
...
fn create_formats() -> EFDResult<HashMap<String, Format>> {
    let parse_color = |hex: &str| {
        u32::from_str_radix(hex, 16).map_err(|e| EFDError::ParseIntError(e, hex.to_string()))
    };
