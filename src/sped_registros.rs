...
use crate::{ALIQ_BASICA_COF, ALIQ_BASICA_PIS, DECIMAL_ALIQ, FATOR_PRECISAO};
...
impl FromStr for IndicadorOrigem {
    type Err = (); // Ou um erro customizado se preferir
...
impl IndicadorOrigem {
    /// Converte u16 para o IndicadorOrigem de forma segura
...
// Permite usar .to_string() ou println!("{}", enum) usando a string definida no serde
impl fmt::Display for IndicadorOrigem {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        // Usa a implementação de Serialize para escrever a string (rename) no formatter
        self.serialize(f)
    }
...
impl Serialize for TipoOperacao {
...
impl fmt::Display for TipoOperacao {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        self.serialize(f)
    }
}
...
/*
impl FromStr for TipoDeCredito {
    type Err = String;

    fn from_str(s: &str) -> Result<Self, Self::Err> {
        // .trim() para garantir que espaços e quebras de linha não quebrem o parse
        match s.trim() {
            "01" | "1" => Ok(Self::AliquotaBasica),
            "02" | "2" => Ok(Self::AliquotasDiferenciadas),
            "03" | "3" => Ok(Self::AliquotaPorUnidadeProduto),
            "04" | "4" => Ok(Self::EstoqueAbertura),
            "05" | "5" => Ok(Self::AquisicaoEmbalagens),
            "06" | "6" => Ok(Self::PresumidoAgroindustria),
            "07" | "7" => Ok(Self::OutrosCreditosPresumidos),
            "08" | "8" => Ok(Self::Importacao),
            "09" | "9" => Ok(Self::AtividadeImobiliaria),
            "99" => Ok(Self::Outros),
            _ => Err(format!("Código de Tipo de Crédito inválido: {}", s)),
        }
    }
}
*/
...
pub fn obter_aliquota_correlacionada_de_pis(aliq_cofins: &str) -> Option<String> {
    match CORRELACAO_DE_ALIQUOTAS.get(&aliq_cofins) {
        ...
        None => {
            ...
            eprintln!(
                "Não foi possível obter diretamente a alíquota de PIS/PASEP relacionada à alíquota de COFINS: {aliq_cofins}"
            );
            eprintln!(
                "A correlação entre as alíquotas de PIS/PASEP e COFINS será obtida da função correlacionar_aliquotas_codcred em dispatch_table.rs.\n"
            );
            None
        }
    }
...
/// Removemos o 'expect' do runtime para garantir que falhas de conversão sejam tratadas,
/// embora literais f64 dificilmente falhem aqui.
static CORRELACAO_COFINS_PIS: LazyLock<HashMap<Decimal, Decimal>> = LazyLock::new(|| {
    [
        ...
        (3.80, 0.8250),
        (4.56, 0.9900),
        (5.70, 1.2375), // Subcontratação Transporte
        (6.08, 1.3200),
        (7.00, 1.6500),
        (7.60, 1.6500),
        (8.54, 1.8600), // Atacadista -> Varejista/Consumidor
        (9.65, 2.1000),
        (10.68, 2.3200),
        (10.80, 2.3000),
        (14.37, 2.1000),
...
pub fn obter_pis_da_tabela_estatica(aliq_cofins: &Decimal) -> Option<Decimal> {
    CORRELACAO_COFINS_PIS.get(aliq_cofins).copied()
}
...
/// Representação em Ponto Fixo (Inteiro) das alíquotas.
///
...
impl AliquotasKey {
    /// Converte f64 para u32 (Ponto Fixo).
    /// #[inline] sugere ao compilador otimizar chamadas em loops.
    #[inline]
    fn from_floats(pis: f64, cof: f64) -> Self {
        Self {
            pis: (pis * FATOR_PRECISAO).round() as u32,
            cof: (cof * FATOR_PRECISAO).round() as u32,
        }
    }
}
...
static ALIQUOTAS_DE_CRED_PRESUMIDO: LazyLock<HashSet<AliquotasKey>> = LazyLock::new(|| {
    ...
    // Programação Funcional: iter -> map -> collect
    percentuais
        .into_iter()
        .map(|p| AliquotasKey::from_floats(p * ALIQ_BASICA_PIS, p * ALIQ_BASICA_COF))
        .collect()
});

/// Verifica se o par de alíquotas corresponde a crédito presumido.
pub fn cred_presumido(aliq_pis: Option<f64>, aliq_cof: Option<f64>) -> bool {
    aliq_pis
        .zip(aliq_cof) // Combina Option<A> e Option<B> em Option<(A, B)>
        .is_some_and(|(pis, cof)| {
            let key = AliquotasKey::from_floats(pis, cof);
