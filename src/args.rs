...
pub struct Arguments {
    ...
    /// Ativar mensagens de debug (ex: Correlações do Bloco M)
    #[arg(short = 'd', long)]
    pub debug: bool,
    ...
    */
    ...
    /// Not Print XLSX (Excel) file.
    ///
    /// Para não imprimir o arquivo .xlsx, adicione a opção: --not_print_xlsx or -n
    #[arg(
        short,
        long,
        value_parser,
...
impl Arguments {
    /// Executa a lógica de validação após o parse
    pub fn build(&mut self) -> EFDResult<()> {
        ...
        if self.find || self.range.is_some() {
            self.search_files(EFD_PATTERN)?;
        } else {
            self.print_help_msg();
            process::exit(1);
        };
        ...
        G: Generator + std::fmt::Debug,
    {
        ...
        log::info!("Generating completion file for {gen:?}...");
