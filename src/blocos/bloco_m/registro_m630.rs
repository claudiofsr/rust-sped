use crate::{
    CodigoDoCredito, EFDError, EFDResult, SpedParser, StringParser, ToCodigoDoCredito, ToDecimal,
    impl_reg_methods,
};
...
const REGISTRO: &str = "M630";

#[derive(Debug, Clone)]
pub struct RegistroM630 {
    /// Nível hierárquico
    pub nivel: u16,
...
impl_reg_methods!(RegistroM630);

impl SpedParser for RegistroM630 {
    type Output = RegistroM630;

    fn parse_reg(file_path: &Path, line_number: usize, fields: &[&str]) -> EFDResult<Self::Output> {
        let len: usize = fields.len();

        // O registro M630 possui 7 campos de dados + 2 delimitadores = 9.
        if len != 9 {
