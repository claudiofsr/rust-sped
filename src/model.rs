...
macro_rules! define_sped_record {
    (
        ...
        /// Enum Principal: Representa os blocos do arquivo EFD Contribuições.
        ///
        /// O uso de Box garante que o tamanho deste Enum na Stack seja mínimo (otimização para Rayon).
        #[derive(Debug, Clone)]
        pub enum SpedRecord {
            $( $bloco(Box<$bloco>), )*
        }
        ...
        impl SpedRecord {
            ...
            pub fn downcast_ref<T: 'static>(&self) -> EFDResult<&T> {
                let any = match self { $( Self::$bloco(b) => b.as_any(), )* };

                any.downcast_ref::<T>().map_loc(|_|
