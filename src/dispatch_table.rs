...
use crate::{
    CODIGO_TIPO_DE_CREDITO, DECIMAL_VALOR, EFDError, EFDResult, TipoDeCredito, TipoDeRateio,
    Tributo::{self, Cofins, Pis},
    obter_aliquota_correlacionada_de_pis, obter_grupo_de_contas,
    structures::info::Info,
};

use claudiofsr_lib::{StrExtension, get_naive_date, thousands_separator};

type FuncaoLerRegistro = fn(&mut Info, HashMap<String, String>) -> EFDResult<()>;

pub fn make_dispatch_table() -> EFDResult<HashMap<&'static str, FuncaoLerRegistro>> {
...
pub fn get_and_parse_usize(valores: &HashMap<String, String>, key: &str) -> EFDResult<usize> {
    valores
        .get(key)
        .ok_or(EFDError::KeyNotFound(key.to_string()))?
        .parse::<usize>()
...
fn ler_registro_0110(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
    info.global.insert(
...
fn ler_registro_0111(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
...
fn ler_registro_0140(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
...
fn ler_registro_0150(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
...
fn ler_registro_0190(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
...
fn ler_registro_0200(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
...
fn ler_registro_0400(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
...
fn ler_registro_0450(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
...
fn ler_registro_0500(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
...
fn ler_registro_a010(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
    info.cnpj_do_estabelecimento = valores["CNPJ"].to_string();
    Ok(())
}

fn ler_registro_a100(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
...
fn ler_registro_a170(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
    ...
    let pairs: [(String, String); 3] = [
        ...
        ("dt_entrada", &info.reg_a100["DT_EXE_SERV"]),
    ]
    .map(|(k, v)| (k.to_string(), v.to_string()));
...
fn ler_registro_c010(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
    info.cnpj_do_estabelecimento = valores["CNPJ"].to_string();
    Ok(())
}

fn ler_registro_c100(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
...
fn obter_chave_de_correlacao_fraca(contrib: Tributo, valores: &HashMap<String, String>) -> String {
    match contrib {
...
|C191|23456789012342|56|1101|6348,55|95,23|6348,55|1,65|||52,38|112233|
...
|C195|23456789012342|56|1101|6348,55|95,23|6348,55|7,6|||241,24|112233|
|C195|23456789012342|56|1101|6348,55|95,23|6348,55|9,6|||241,24|112234|
...
fn ler_registro_c180(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
    info.reg_c180 = valores; // reter info para posterior uso por reg C185
    info.correlacao = HashMap::new();
    Ok(())
}

fn ler_registro_c181(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
...
fn ler_registro_c190(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
...
fn ler_registro_c191(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
    let array: [String; 2] = [valores["ALIQ_PIS"].clone(), valores["VL_PIS"].clone()];
...
fn ler_registro_c199(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
...
fn ler_registro_c380(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
    info.reg_c380 = valores; // reter info para posterior uso por reg C385
    info.correlacao = HashMap::new();
    Ok(())
}

fn ler_registro_c381(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
...
fn ler_registro_c395(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
...
fn ler_registro_c400(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
    info.reg_c400 = valores; // reter info para posterior uso por reg C485
    info.correlacao = HashMap::new();
    Ok(())
}

fn ler_registro_c405(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
    info.reg_c405 = valores; // reter info para posterior uso por reg C485
    Ok(())
}

fn ler_registro_c481(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
    let chave = obter_chave_de_correlacao_fraca(Pis, &valores);
    let array = [valores["ALIQ_PIS"].clone(), valores["VL_PIS"].clone()];
    info.correlacao.insert(chave, array);
    Ok(())
...
fn ler_registro_c490(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
    info.reg_c490 = valores; // reter info para posterior uso por reg C495
    info.correlacao = HashMap::new();
    Ok(())
}

fn ler_registro_c491(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
    let chave = obter_chave_de_correlacao_fraca(Pis, &valores);
    let array = [valores["ALIQ_PIS"].clone(), valores["VL_PIS"].clone()];
    info.correlacao.insert(chave, array);
    Ok(())
...
fn ler_registro_c500(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
    info.reg_c500 = valores; // reter info para posterior uso por reg C505
    info.correlacao = HashMap::new();
    Ok(())
}

fn ler_registro_c501(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
    let chave = obter_chave_de_correlacao_fraca(Pis, &valores);
    let array = [valores["ALIQ_PIS"].clone(), valores["VL_PIS"].clone()];
    info.correlacao.insert(chave, array);
    Ok(())
...
fn ler_registro_c600(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
    info.reg_c600 = valores; // reter info para posterior uso por reg C605
    info.correlacao = HashMap::new();
    Ok(())
}

fn ler_registro_c601(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
    let chave = obter_chave_de_correlacao_fraca(Pis, &valores);
    let array = [valores["ALIQ_PIS"].clone(), valores["VL_PIS"].clone()];
    info.correlacao.insert(chave, array);
    Ok(())
...
fn ler_registro_c860(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
...
fn ler_registro_d010(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
    info.cnpj_do_estabelecimento = valores["CNPJ"].to_string();
    Ok(())
}

fn ler_registro_d100(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
    info.reg_d100 = valores; // reter info para posterior uso por reg D105
    info.correlacao = HashMap::new();
    Ok(())
}

fn ler_registro_d101(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
    let chave = obter_chave_de_correlacao_fraca(Pis, &valores);
    let array = [valores["ALIQ_PIS"].clone(), valores["VL_PIS"].clone()];
    info.correlacao.insert(chave, array);
    Ok(())
...
fn ler_registro_d200(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
    info.reg_d200 = valores; // reter info para posterior uso por reg D205
    info.correlacao = HashMap::new();
    Ok(())
}

fn ler_registro_d201(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
    let mut hmap = HashMap::new();
...
fn ler_registro_d500(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
    info.reg_d500 = valores; // reter info para posterior uso por reg D505
    info.correlacao = HashMap::new();
    Ok(())
}

fn ler_registro_d501(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
    let chave = obter_chave_de_correlacao_fraca(Pis, &valores);
    let array = [valores["ALIQ_PIS"].clone(), valores["VL_PIS"].clone()];
    info.correlacao.insert(chave, array);
    Ok(())
...
fn ler_registro_d600(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
    info.reg_d600 = valores; // reter info para posterior uso por reg D605
    info.correlacao = HashMap::new();
    Ok(())
}

fn ler_registro_d601(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
    let chave = obter_chave_de_correlacao_fraca(Pis, &valores);
    let array = [valores["ALIQ_PIS"].clone(), valores["VL_PIS"].clone()];
    info.correlacao.insert(chave, array);
    Ok(())
...
fn ler_registro_f010(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
    info.cnpj_do_estabelecimento = valores["CNPJ"].to_string();
    Ok(())
}
...
fn ler_registro_i010(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
    info.cnpj_do_estabelecimento = valores["CNPJ"].to_string();
    Ok(())
}
...
fn ler_registro_m100(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
    info.reg_m100.clone_from(&valores); // reter info para posterior uso no reg M105

    let mut hmap = HashMap::new();
    let linha_da_efd = get_and_parse_usize(&valores, "linha_da_efd")?;

    let mut ajuste_acres: f64 = valores["VL_AJUS_ACRES"].parse::<f64>().unwrap_or_default();
    let mut ajuste_reduc: f64 = valores["VL_AJUS_REDUC"].parse::<f64>().unwrap_or_default();
    let mut cred_descont: f64 = valores["VL_CRED_DESC"].parse::<f64>().unwrap_or_default();
...
fn ler_registro_m105(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
    let mut hmap = HashMap::new();
...
fn codigo_do_tipo_de_credito(
    ...
    // 1. Pipeline de Extração e Validação Pura
    let credito_validado = hmap
        .get("COD_CRED")
        .and_then(|s| s.parse::<u16>().ok())
        // Valida a parte inteira (Rateio: 1 a 4)
        .filter(|&cod| TipoDeRateio::from_u16(cod / 100).is_some())
        // Valida e converte a parte decimal (Crédito: 1 a 99)
        .and_then(|cod| TipoDeCredito::from_u16(cod % 100));

    // 2. Tratamento de Efeitos Colaterais
    if let Some(credito) = credito_validado {
        // Sucesso: Atualiza contexto de importação se necessário
        if credito == TipoDeCredito::Importacao {
            hmap.insert("IND_ORIG_CRED".to_string(), "1".to_string());
        }
        Some(credito)
    } else {
        // Falha: Log de erro (Cold Path)
        let arquivo = info
            .global
            .get("arquivo_efd")
            .map(|s| s.as_str())
...
fn ler_registro_m500(info: &mut Info, valores: HashMap<String, String>) -> EFDResult<()> {
...
fn ler_registro_de_controle_de_creditos_fiscais(
    ...
    let contribuicao: Tributo = match valores["REG"].as_ref() {
        "1100" => Pis,
        "1500" => Cofins,
        _ => return Ok(()),
    };
